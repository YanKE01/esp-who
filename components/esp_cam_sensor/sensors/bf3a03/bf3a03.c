...
/*
 * BF3A03 camera sensor special effect.
 */
typedef enum {
    BF3A03_EFFECT_ENC_NORMAL,
    BF3A03_EFFECT_ENC_GRAYSCALE,
    BF3A03_EFFECT_ENC_SEPIA,
    BF3A03_EFFECT_ENC_SEPIAGREEN,
    BF3A03_EFFECT_ENC_SEPIABLUE,
    BF3A03_EFFECT_ENC_COLORINV,
} bf3a03_spec_effect_mode_t;

/*
 * BF3A03 camera sensor scene mode.
 */
typedef enum {
    BF3A03_SCENE_MODE_NORMAL,
    BF3A03_SCENE_MODE_NIGHT,
} bf3a03_scene_mode_t;
...
#define BF3A03_AEC_TARGET_MAX (0x60)
...
static const esp_cam_sensor_format_t bf3a03_format_info[] = {
    {
        ...
        .width = 640,
        .height = 480,
        .regs = DVP_8bit_20Minput_640x480_yuv422_20fps_color,
        .regs_size = ARRAY_SIZE(DVP_8bit_20Minput_640x480_yuv422_20fps_color),
        .fps = 20,
        .isp_info = NULL,
        .mipi_info = {},
        .reserved = NULL,
...
static esp_err_t bf3a03_set_antibanding(esp_cam_sensor_device_t *dev, int val)
{
    ...
    case BF3A03_BANDING_50HZ:
        ret = bf3a03_write(dev->sccb_handle, 0x07, 0x92); //Bit[3:2]: the bigger, Y_AVER_MODIFY is smaller,BIT[1]:banding filter selection
        ret |= bf3a03_write(dev->sccb_handle, 0x0c, 0x81); //50hz banding 0x78
        break;
    case BF3A03_BANDING_60HZ:
        ret = bf3a03_write(dev->sccb_handle, 0x07, 0x90); //Bit[3:2]: the bigger, Y_AVER_MODIFY is smaller,BIT[1]:banding filter selection
        ret |= bf3a03_write(dev->sccb_handle, 0x0d, 0x6b); //60hz banding 0x64
        break;
    default:
        break;
    }
...
static esp_err_t bf3a03_set_wb_mode(esp_cam_sensor_device_t *dev, int val)
{
    ...
    case BF3A03_WB_AUTO:/* auto */
        ret |=  bf3a03_write(dev->sccb_handle, 0xb2, 0x89);
        break;
    case BF3A03_WB_CLOUD: /* cloud */
        ret |=  bf3a03_write(dev->sccb_handle, 0xb2, 0x81);
        ret |=  bf3a03_write(dev->sccb_handle, 0xb3, 0x55);
        ret |=  bf3a03_write(dev->sccb_handle, 0xb0, 0x10);
        ret |=  bf3a03_write(dev->sccb_handle, 0xb1, 0x20);
        break;
    case BF3A03_WB_DAYLIGHT://sunny
        ret |=  bf3a03_write(dev->sccb_handle, 0xb2, 0x81);
        ret |=  bf3a03_write(dev->sccb_handle, 0xb3, 0x55);
        ret |=  bf3a03_write(dev->sccb_handle, 0xb0, 0x13);
        ret |=  bf3a03_write(dev->sccb_handle, 0xb1, 0x19);
        break;
    case BF3A03_WB_INCANDESCENCE://office
        ret |=  bf3a03_write(dev->sccb_handle, 0xb2, 0x81);
        ret |=  bf3a03_write(dev->sccb_handle, 0xb3, 0x55);
        ret |=  bf3a03_write(dev->sccb_handle, 0xb0, 0x14);
        ret |=  bf3a03_write(dev->sccb_handle, 0xb1, 0x0d);
        break;
    case BF3A03_WB_TUNGSTEN://home
        ret |=  bf3a03_write(dev->sccb_handle, 0xb2, 0x81);
        ret |=  bf3a03_write(dev->sccb_handle, 0xb3, 0x55);
        ret |=  bf3a03_write(dev->sccb_handle, 0xb0, 0x1a);
        ret |=  bf3a03_write(dev->sccb_handle, 0xb1, 0x12);
        break;
    case BF3A03_WB_FLUORESCENT:
        ret |=  bf3a03_write(dev->sccb_handle, 0xb2, 0x81);
        ret |=  bf3a03_write(dev->sccb_handle, 0xb3, 0x55);
        ret |=  bf3a03_write(dev->sccb_handle, 0xb0, 0x1f);
        ret |=  bf3a03_write(dev->sccb_handle, 0xb1, 0x1a);
        break;
...
static esp_err_t bf3a03_set_effect(esp_cam_sensor_device_t *dev, int val)
{
    ...
    switch (val) {
    case BF3A03_EFFECT_ENC_NORMAL:
        effect_array_index = 0;
        break;
    case BF3A03_EFFECT_ENC_GRAYSCALE:
        effect_array_index = 1;
        break;
    case BF3A03_EFFECT_ENC_SEPIA:
        effect_array_index = 2;
        break;
    case BF3A03_EFFECT_ENC_SEPIAGREEN:
        effect_array_index = 3;
        break;
    case BF3A03_EFFECT_ENC_SEPIABLUE:
        effect_array_index = 4;
        break;
    case BF3A03_EFFECT_ENC_COLORINV:
...
static esp_err_t bf3a03_set_scene_mode(esp_cam_sensor_device_t *dev, int val)
{
    esp_err_t ret = ESP_OK;

    switch (val) {
    case BF3A03_SCENE_MODE_NIGHT:
        ret |= bf3a03_write(dev->sccb_handle, 0x09, 0xa1);
        break;
    case BF3A03_SCENE_MODE_NORMAL://SCENE_MODE_AUTO:
        ret |= bf3a03_write(dev->sccb_handle, 0x09, 0x8b);
        break;
    default:
        break;
    }
...
static esp_err_t bf3a03_query_para_desc(esp_cam_sensor_device_t *dev, esp_cam_sensor_param_desc_t *qdesc)
{
    ...
    case ESP_CAM_SENSOR_SPECIAL_EFFECT:
        qdesc->type = ESP_CAM_SENSOR_PARAM_TYPE_NUMBER;
        qdesc->number.minimum = BF3A03_EFFECT_ENC_NORMAL;
        qdesc->number.maximum = BF3A03_EFFECT_ENC_COLORINV;
        qdesc->number.step = 1;
        qdesc->default_value = BF3A03_EFFECT_ENC_NORMAL;
        break;
    case ESP_CAM_SENSOR_SCENE:
        qdesc->type = ESP_CAM_SENSOR_PARAM_TYPE_NUMBER;
        qdesc->number.minimum = BF3A03_SCENE_MODE_NORMAL;
        qdesc->number.maximum = BF3A03_SCENE_MODE_NIGHT;
        qdesc->number.step = 1;
        qdesc->default_value = BF3A03_SCENE_MODE_NORMAL;
        break;
...
static esp_err_t bf3a03_set_para_value(esp_cam_sensor_device_t *dev, uint32_t id, const void *arg, size_t size)
{
    ...
    case ESP_CAM_SENSOR_SCENE: {
        ...
        ret = bf3a03_set_scene_mode(dev, *value);
        break;
    }
    case ESP_CAM_SENSOR_AE_FLICKER: {
        int *value = (int *)arg;
